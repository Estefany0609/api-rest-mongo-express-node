...
export const createPosition = async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO web_financial.portfolio_user ( uid, ticker, entry_date, entry_price, position_type, qyt ) VALUES ($1, $2, $3, $4, $5, $6)',
            [req.uid, ticker, entry_date, entry_price, position_type, qyt])
        
        return res.status(201).json({ ticker, entry_date, entry_price, position_type, qyt })      
...
export const removePosition = async (req, res) => {
    try {
        ...
        const { id } = req.params
        const position = await pool.query('SELECT * FROM web_financial.portfolio_user WHERE id = $1 ', [id])

        if (!position.rows[0]) return res.status(404).json({ error: "No existe el ticker" })

        const respuestaPassword = await position.rows[0].uid === req.uid

        if(!respuestaPassword) return res.status(401).json({ error: "No le pertenece esa position" });
        
        await pool.query('DELETE FROM web_financial.portfolio_user WHERE id = $1 ', [id])
...
export const updatePosition = async (req, res) => {
    try {
        ...
        if (position_type === "SHORT") { qyt = -qyt }
        ...
        await pool.query('UPDATE web_financial.portfolio_user SET entry_date = $2, entry_price = $3, position_type= $4, qyt= $5 WHERE id = $1 ', [ id, entry_date, entry_price, position_type, qyt  ])
